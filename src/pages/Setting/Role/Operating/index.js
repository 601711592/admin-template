...
class Operating extends Component {
  ...
  getParentArr = id => {
    ...
    if (r) {
      list.push(id);
      if (r.parent_id) {
        list = list.concat(this.getParentArr(r.parent_id));
      }
    }
    return list;
  };
  ...
  getChildren = id => {
    let list = [];
    const arr = this.props.menuPlane.filter(v => v.parent_id === id);
    arr.map(v => {
      list.push(v.id);
  ...
  render() {
    ...
    const modalProps = {
      ...
      onOk: () => {
        validateFields((err, values) => {
          if (!err) {
            onSubmit({ ...values, menuIds: values.menuIds.join() }, () => {
              resetFields();
              onCancel();
            });
          }
    ...
    const rowSelection = {
      ...
      onSelect: (records, selected, selectedRows) => {
        ...
        let values = [];
        if (selected) {
          const arr = this.getParentArr(records.parent_id);
          arr.map(v => {
            if (!selectedRowIds.find(_ => _ === v)) {
              selectedRowIds.push(v);
            }
            return null;
          });
